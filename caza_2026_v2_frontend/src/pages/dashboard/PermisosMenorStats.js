...
import { fetchPermisosMenorStats } from '../../utils/api';
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer } from 'recharts';
import '../../styles/App.css';

const PermisosMenorStats = () => {
  ...
  const getStats = async () => {
    try {
      const data = await fetchPermisosMenorStats();
      setStats(data);
    } catch (err) {
      setError('No se pudieron cargar las estadísticas de permisos de caza menor.');
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  if (loading && stats.total_permisos === 0) {
    return <p>Cargando estadísticas de permisos de caza menor...</p>;
  }
  ...
  return (
    <div className="stats-container">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        <h2>Estadísticas de Permisos Caza Menor</h2>
        <button
          onClick={getStats}
          disabled={loading}
          style={{
      ...
      <div className="stat-widget-container">
        <div className="stat-widget">
          ...
          <div className="stat-widget-label">Total de Permisos Emitidos</div>
        </div>
      </div>
...
export default PermisosMenorStats;
